...
from fpl_ai.historical_transform import read_source_csv
...
def snapshot_bytes(
    *,
    ...
    gameweek_one_is_next: bool = True,
    gameweek_two_is_next: bool = False,
) -> bytes:
    payload = {
        "events": [
            {
                ...
                "is_next": gameweek_one_is_next,
            },
            {
                "id": 2,
                "deadline_time": "2024-08-23T18:00:00Z",
                "is_next": gameweek_two_is_next,
            },
        ],
        "elements": [
            {
...
def synthetic_sources() -> tuple[dict[str, bytes], dict]:
    ...
    post_deadline_path = "cache/2024/8/23/1900.json.xz"
    missing_gw_path = "cache/2024/8/23/1700.json.xz"
    tree = {
        "truncated": False,
        "tree": [
            {"path": path, "type": "blob"}
    ...
    values = {
        ...
        "https://example.test/tree": json.dumps(tree).encode(),
        ...
        f"https://example.test/fplcache/{CACHE_REVISION}/{missing_gw_path}": snapshot_bytes(
            gameweek_one_is_next=False, gameweek_two_is_next=False
        ),
    }
...
class HistoricalPipelineTests(unittest.TestCase):
    ...
    def test_pipeline_provenance_leakage_and_snapshot_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result.snapshot_gameweeks, 1)
            self.assertEqual(result.missing_snapshot_gameweeks, (2,))
            ...
            self.assertEqual(zero_minute["team_id"], "1")
            self.assertEqual(zero_minute["opponent_team_id"], "2")

            with (result.processed_dir / "player_deadline_snapshots.csv").open(newline="") as handle:
                snapshots = list(csv.DictReader(handle))
            self.assertEqual(len(snapshots), 2)
            self.assertEqual({row["capture_time_utc"] for row in snapshots}, {"2024-08-16T16:00:00Z"})
            injured = next(row for row in snapshots if row["element"] == "102")
            ...
            self.assertEqual(gameweeks[1]["valid_predeadline_snapshot"], "False")
            self.assertEqual(gameweeks[1]["hours_before_deadline"], "")
            ...
            self.assertEqual(manifest["source_revisions"]["vaastav"], VAASTAV_REVISION)
