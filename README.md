...
Provenance has two deliberately separate hashes. `source_identity_sha256` identifies the pinned providers and exact immutable raw artifacts, including their content hashes. `build_identity_sha256` identifies the transformation contract, canonical schema contract, season expectations, snapshot-selection settings, selected source schema, and reconciliation policy. Timestamps and local output paths are excluded from the build hash. This permits two semantic builds from the same sources without overwriting one another.
...
Vaastav schemas are registered by schema ID and restricted to declared seasons. The 2024/25 definition records disjoint required, optional, ignored, quarantined, and forbidden categories; trusted and quarantine mappings; expected types and metrics; and known exceptions. Validation rejects duplicate or overlapping declarations, undeclared mapping sources, ambiguous canonical targets, malformed schemas, and every trusted mapping from a forbidden field. `xP` is structurally forbidden from mapping to any trusted expected-points output. Missing required columns or an unknown/inapplicable schema fails clearly. Optional removals are reported and remain null; unexpected additions or order drift are reported as quality failures. Adding 2023/24 next requires a new catalogue entry and source-schema definition, not a change to generic reconciliation logic.

Every new processed build owns `source_inventory.json`, an atomic frozen copy of the precise inventory used for that build. Its portable relative path, SHA-256, full source identity, and source-identity hash are recorded in the processed manifest and catalogue. Raw files remain shared without duplication, but later changes to the raw cache inventory cannot change or invalidate a frozen build's provenance. Catalogue schema v2 retains a version-indexed build history as well as the latest-successful lookup.
